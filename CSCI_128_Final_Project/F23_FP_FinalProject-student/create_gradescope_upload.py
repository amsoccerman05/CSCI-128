...
def addFolderToZip(_currentZipBuffer: ZipFile, _directoryToAdd: str) -> None:
    ...
    for file in os.listdir(_directoryToAdd):
        # For this autograder, we want to take all of the files that the student has
        # we are ignoring any dotfiles
        if os.path.isfile(_directoryToAdd + file) and file[0] != ".":
