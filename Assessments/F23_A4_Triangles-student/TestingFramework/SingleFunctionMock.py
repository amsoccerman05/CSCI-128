class SingleFunctionMock:
    ...
    def assertCalledWith(self, *args, message=""):
        self.assertCalled()
        for calledWithArgs in self.calledWithArgs:
            if calledWithArgs == args:
                return

        raise AssertionError(f"Function: {self.name} was not called with arguments: {self.name}{args}.\n"
                             f"{message}")

    def assertCalledTimes(self, _times: int):
        if _times != self.calledTimes:
            raise AssertionError(
                f"Function: {self.name} was called {self.calledTimes} times.\n"
                f"Expected to be called {_times} times.")
